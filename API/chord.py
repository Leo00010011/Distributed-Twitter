...
class ChordNode:
    ...
    def __init__(self, n, nodeSet, nbits):
        self.FT= [None] *n #La tabla de nodos
        self.nodeSet = nodeSet
        self.nodeID =  random.randint(2**nbits) #es ID de esta maquina
        self.nBits= nbits
        self.MAXPROC = 2**nbits #esto es el numero mas grande de IP en el chord
    ...
    def finger(self, i):
        ...
        for k in range(len(self.nodeSet)):# process segments
            if self.inbetween(succ, self.nodeSet[lwbi]+1, self.nodeSet[upbi]+1, len(self.nodeSet)):
                return self.nodeSet[upbi] # found successor
            
            (lwbi,upbi) = (upbi, (upbi+1) % len(self.nodeSet)) # next segment
    
    def inbetween(x, lbID, ubID,max):
        return x > lbID and x%max < ubID
