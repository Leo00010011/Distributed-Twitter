...
class ChordServer:
    def __init__(self,DHT_name,port,disable_log,is_the_first = False):
        ...
        self.server : MultiThreadedServer = MultiThreadedServer(self.port,10,10,2,self.create_dispatcher(), log = False)
        ...
        with self.log_lock:
            ...
            if(not(self.disable_realtime_log == 'yes')):
                if(self.disable_realtime_log == 'file'):
                    with open(f'node_{self.id_hex}_log','a') as f:
    ...
    def succ_who(self,k,as_max) -> ChordNode:
        ...
        if less_than_me and self.Ft[-1].as_max:
            ...
            num = self.Ft[-1].id - self.max_id
            num_hex = hex(num)[2:]
            return ChordNode(num,num_hex,self.Ft[-1].ip,False)
        else:
    ...
    def ask_succ(self, node_ip:str, id_hex:int, as_max:bool) -> ChordNode:
        holder = self.state_storage.insert_state()
        for _ in range(10):
            msg = ChordServer.create_msg(self.get_succ_req_cmd,id_hex,self.ip,as_max,holder.id)
            self.update_log(f'starting to send (ask_succ to {node_ip} for {id_hex} as_max:{str(as_max)})')
